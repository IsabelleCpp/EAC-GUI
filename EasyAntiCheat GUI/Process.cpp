...
//bool ProcessData::operator==(const PROCESSENTRY32& rhs) const
//{
//	return (m_PID == rhs.th32ProcessID);
//}

//bool ProcessData::operator==(const PROCESSENTRY32W& rhs) const
//{
//	return (m_PID == rhs.th32ProcessID);
//}
...
bool GetProcessList(std::vector<ProcessData *> & list)
{
	...
	for (auto iter = entries.begin(); iter != entries.end(); )
	{
		...
		{
			return (i.th32ProcessID == (entry->m_PID));
		};
	...
	for (UINT i = 0; i < entries.size(); )
	{
		...
		if (current_entry.m_PID != entries[i].th32ProcessID)
		{
			delete list[i];
